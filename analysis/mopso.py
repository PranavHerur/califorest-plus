...
class Particle:
    ...
    """
    def __init__(self, bounds: Dict[str, Tuple], velocity_factor: float = 0.1, random_state: int = None):
        """
        ...
        for param_name, (min_val, max_val) in bounds.items():
            if param_name == 'max_features':
                ...
                self.velocity[param_name] = 0  # For categorical variables, velocity is a probability
                ...
                self.velocity[param_name] = np.random.uniform(-max_velocity, max_velocity)
            else:
                # Float parameters
    ...
    def update_velocity(self, global_best_position: Dict[str, Any], 
        ...
        for param_name in self.position:
            if param_name == 'max_features':
                ...
                cognitive_velocity = c1 * r1 * (self.best_position[param_name] - self.position[param_name])
                social_velocity = c2 * r2 * (global_best_position[param_name] - self.position[param_name])
                
                self.velocity[param_name] = w * self.velocity[param_name] + cognitive_velocity + social_velocity
    ...
    def update_position(self):
        ...
        for param_name in self.position:
            if param_name == 'max_features':
                ...
                if self.velocity[param_name] != 0:  # Only change if velocity indicates change
...
class MOPSO:
    ...
    """
    def __init__(self, bounds: Dict[str, Tuple], n_particles: int = 20, 
                 max_iter: int = 30, w: float = 0.7, c1: float = 1.5, c2: float = 1.5,
                 repository_size: int = 50, mutation_rate: float = 0.1, 
                 leader_selection_method: str = 'crowding', random_state: Optional[int] = None):
        """
    ...
    def evaluate_particle(self, particle: Particle, X_train, y_train, X_val=None, y_val=None, 
                          cv: int = 5) -> List[float]:
        """
    ...
    def optimize(self, X_train, y_train, X_val=None, y_val=None, cv: int = 5, 
                 verbose: bool = True) -> Tuple[List[Dict[str, Any]], List[List[float]]]:
        """
        ...
        for particle in self.particles:
            fitness = self.evaluate_particle(particle, X_train, y_train, X_val, y_val, cv)
        ...
        for iteration in range(self.max_iter):
            ...
            for particle in self.particles:
                # Select a leader from the repository
                leader_position = self.repository.select_leader(method=self.leader_selection_method)
            ...
            if verbose:
                ...
                if len(positions) > 0:
                    ...
                    print(f"Average AUC: {avg_auc:.4f}, Brier: {avg_brier:.4f}, Time: {avg_time:.2f}s")
                    ...
                    print(f"Best AUC: {fitnesses[best_auc_idx][0]:.4f} - {positions[best_auc_idx]}")
                    print(f"Best Brier: {fitnesses[best_brier_idx][1]:.4f} - {positions[best_brier_idx]}")
                    print(f"Best Time: {fitnesses[best_time_idx][2]:.2f}s - {positions[best_time_idx]}")
    ...
    def get_best_solution(self, criterion: str = 'auc') -> Tuple[Dict[str, Any], List[float]]:
        """
        ...
        if criterion == 'auc':
            ...
            brier_norm = 1 - (briers - np.min(briers)) / (np.max(briers) - np.min(briers) + 1e-10)
            time_norm = 1 - (times - np.min(times)) / (np.max(times) - np.min(times) + 1e-10)
