...
class ImprovedVennAbersForest(ClassifierMixin, BaseEstimator):
    ...
    def _improved_isotonic_calibration(self, scores, labels, sample_weight=None):
        ...
        if sample_weight is not None:
            ...
            extended_weights = np.concatenate([[max_weight], sample_weight, [max_weight]])
        else:
    ...
    def _compute_calibration_weights(self, scores, labels, variance=None):
        ...
        confidence_weights = 1 - 0.3 * confidence  # Less weight for very confident predictions
    ...
    def fit(self, X, y):
        ...
        skf = StratifiedKFold(n_splits=self.calibration_folds, shuffle=True, random_state=42)
        ...
        for train_idx, cal_idx in skf.split(X, y):
            ...
            cal_weights = self._compute_calibration_weights(cal_scores, y_cal, cal_variance)
    ...
    def predict_proba(self, X):
        ...
        for i, (p0_cal, p1_cal) in enumerate(zip(self.p0_calibrators_, self.p1_calibrators_)):
...
class VennAbersForest(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        kf = StratifiedKFold(n_splits=self.calibration_folds, shuffle=True, random_state=42)
...
class BayesianVennAbersForest(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        unique_y = np.unique(y)
        if len(unique_y) != 2:
            raise ValueError("BayesianVennAbersForest only supports binary classification.")
        ...
        for i in range(self.n_estimators):
            ...
            if self.class_weight == 'balanced':
                ...
                pos_sample = resample(pos_idx, replace=True, n_samples=len(pos_idx), random_state=i)
                neg_sample = resample(neg_idx, replace=True, n_samples=len(neg_idx), random_state=i)
                bootstrap_idx = np.concatenate([pos_sample, neg_sample])
            else:
                # Regular bootstrap
                bootstrap_idx = resample(indices, replace=True, n_samples=n_samples, random_state=i)
    ...
    def predict_proba(self, X):
        ...
        if hasattr(self, 'second_calibrator_'):
            if self.second_stage_calibration == 'sigmoid':
                final_probs = self.second_calibrator_.predict_proba(va_probs.reshape(-1, 1))[:, 1]
            elif self.second_stage_calibration == 'isotonic':
    ...
    def get_uncertainty_ranges(self, X):
        ...
        if hasattr(self, 'second_calibrator_'):
            if self.second_stage_calibration == 'sigmoid':
                lower_bound = self.second_calibrator_.predict_proba(lower_bound.reshape(-1, 1))[:, 1]
                upper_bound = self.second_calibrator_.predict_proba(upper_bound.reshape(-1, 1))[:, 1]
            elif self.second_stage_calibration == 'isotonic':
